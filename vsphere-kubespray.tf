...
resource "null_resource" "kubespray_create" {
  ...
  provisioner "local-exec" {
    command = "cp -rf missing/kubespray-default.yml kubespray/roles/kubespray-defaults/defaults/main.yml && cp -rf missing/vsphere-cloud-config.j2 kubespray/roles/kubernetes/node/templates/vsphere-cloud-config.j2 && cd kubespray && ansible-playbook -i ../config/hosts.ini -b -u ${var.vm_user} -v cluster.yml -e kube_version=${var.k8s_version}"
  }
